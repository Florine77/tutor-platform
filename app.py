...
def add_student():
    ...
    db = get_db()
    cursor = db.execute('''
        INSERT INTO students (name, grade, subject, weaknesses, goal, personality, attention, device, parent_contact, parent_notes)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
def update_student(id):
    ...
    db = get_db()
    db.execute('''
        UPDATE students SET name=?, grade=?, subject=?, weaknesses=?, goal=?, personality=?, attention=?, device=?, parent_contact=?, parent_notes=?
        WHERE id=?
    ''', (data['name'], data.get('grade',''), data.get('subject',''), data.get('weaknesses',''),
          data.get('goal',''), data.get('personality',''), data.get('attention',''),
          data.get('device',''), data.get('parent_contact',''), data.get('parent_notes',''), id))
    db.commit()
    db.close()
...
⚠️ 需要加强：{not_mastered if not_mastered else '暂无特别薄弱环节'}
